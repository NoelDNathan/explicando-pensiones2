...
1. Mantener una identidad visual coherente: usar los colores, tipografias, espaciados y radios definidos como variables o tokens globales antes de introducir nuevos valores.
   Hay tres paletas y conviene no confundirlas: `:root` en `src/index.css` para las paginas del sitio; el `@theme` de `src/index.css` (`--color-*`), que es la paleta oscura de las paginas de graficos; y los tokens `--fiscal-*` de `FiscalSoftTheme.css` para la calculadora fiscal. Si el color no existe como token, anyadir el token, no el literal. `pnpm verify:styles` falla si un archivo CSS gana colores escritos a mano.
   **Calculadora fiscal:** los componentes de la calculadora se escriben directamente con `var(--fiscal-*)`, sin CSS oscuro de base. No se crea un estilo "por defecto" y luego una regla `.fwd--soft .x` que lo repinte, ni se ponen valores de reserva a otra paleta (`var(--fiscal-ink, var(--text))`). Para acentos, usar primero los tokens de significado (`--fiscal-worker`, `-company`, `-state`, `-region`, `-positive`, `-negative`) y solo despues el color crudo (`--fiscal-blue`, `-violet`...) con sus variantes `-soft`, `-line` e `-ink`. En `/componentes`, las piezas de la calculadora se muestran dentro de `component-preview--calculadora fwd--soft fwd-worker-card`, igual que en la pagina real. `pnpm verify:styles` falla sin linea base si el CSS de la calculadora tiene un color a mano, un token de otra paleta o una regla `.fwd--soft`, o si un componente TS/TSX de la calculadora escribe un color o usa clases de color de Tailwind.
